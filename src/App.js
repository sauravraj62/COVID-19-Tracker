...
import RegionSelector from './Components/RegionDropDown/RegionSelector'
class App extends Component {
  state = {
    ...
    DeathCount : 0,
    LastUpdated : '',
    Countries : [{name: "Global", iso2: "Global", iso3: "Global"},
    ...
    {name: "Czechia", iso2: "CZ", iso3: "Czechia"},
  ...
  componentDidMount = () => {
    const response = axios.get('https://covid19.mathdro.id/api');
    response.then(res => {
      this.setState({ConfirmedCount : res.data.confirmed.value});
    });
    response.then(res => {
      this.setState({RecoveredCount : res.data.recovered.value});
    });
    response.then(res => {
      this.setState({DeathCount : res.data.deaths.value});
    });
    response.then(res => {
      this.setState({LastUpdated : res.data.lastUpdate});
    });
  }

  resultUpdateHandler = (confirmed,recovered, deaths, lastUpdate ) => {
      this.setState({ConfirmedCount : confirmed});
      this.setState({RecoveredCount : recovered});
      this.setState({DeathCount : deaths});
      this.setState({LastUpdated : lastUpdate});
  }
  ...
  render () {
    return (
      <div className="App">
        ...
        updateHandler={this.resultUpdateHandler} />
